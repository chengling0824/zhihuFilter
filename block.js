...
function getBlockedUser(){
	$.get("/settings/filter").done(function(data){
		var html = $(data);
		html.find('.BlockedUsers-descrUsers .UserPageItem-link').each(function(){
...
function blockUser(){
	...
	//发现页屏蔽
	$('.tab-panel .feed-item').each(function(){
		var $self = $(this);
		var href = $self.find('.author-link-line .author-link').text();
		if( blockedUsers.indexOf(href) != -1 ){
			$self.hide();
			blockedItems.push($self);
		}
    ...
    //评论屏蔽
	$('.CommentList .CommentItem').each(function(){
		var $self = $(this);
		var href = $self.find('.CommentItem-meta .UserLink-link').text();
		if( blockedUsers.indexOf(href) != -1 ){
			$self.hide();
			blockedItems.push($self);
		}
